...
import { listBrands, addBrand, removeBrand } from "../brands/registry.js";
import { initBrandVaults, getBrandVaultSummary, readVault, saveToVault, searchVaults, } from "../vault/index.js";
        ...
        switch (name) {
            case "blume_generate_post": {
                const post = await generatePost({
                    ...
                    goal: args.goal,
                    aidaStage: args.aida_stage ?? "attention",
                    mode: args.mode ?? "collaborative",
            ...
            case "blume_aida_sequence": {
                const posts = await generateAIDASequence(String(args.brand), args.platform, args.goal, args.mode ?? "collaborative", args.topic ? String(args.topic) : undefined);
            ...
            case "blume_add_brand": {
                const profile = {
                    ...
                    voice: {
                        ...
                        do: Array.isArray(args.voice_do) ? args.voice_do : ["Be authentic"],
                        dont: Array.isArray(args.voice_dont) ? args.voice_dont : ["Don't be generic"],
                        sample_phrases: Array.isArray(args.sample_phrases) ? args.sample_phrases : [],
                    },
                    aida: {
                    ...
                    platforms: Array.isArray(args.platforms) ? args.platforms : ["twitter"],
                };
                if (args.entity)
                    profile.entity = String(args.entity);
                addBrand(profile);
            ...
            case "vault_search": {
                const vault = args.vault ? Number(args.vault) : undefined;
