...
function log(msg) { console.log(`[BLUME] ${msg}`); }
function err(msg) { console.error(`[BLUME ERROR] ${msg}`); }
function cmdInitVaults() {
    ...
    for (const slug of brands)
        initBrandVaults(slug);
    log(`Initialized vaults for ${brands.length} brands: ${brands.join(", ")}`);
}
async function generatePostForBrand(brandSlug, platform, goal, aidaStage, mode) {
...
async function cmdBulkGenerate(postCount, offerCount, emailCount) {
    ...
    const modes = ["collaborative", "dominion_rex", "venus_protocol", "commerce", "insight", "grace"];
    ...
    for (const brandSlug of brands) {
        ...
        for (let i = 0; i < postCount; i++) {
            ...
            try {
                ...
                err(`    [${i + 1}/${postCount}] FAILED: ${e.message}`);
            }
        }
        ...
        for (let i = 0; i < offerCount; i++) {
            try {
                ...
                err(`    [${i + 1}/${offerCount}] FAILED: ${e.message}`);
            }
        }
        ...
        for (let i = 0; i < emailCount; i++) {
            ...
            try {
                ...
                err(`    [${i + 1}/${emailCount}] FAILED: ${e.message}`);
            }
        }
    }
...
function cmdExportAll() {
    ...
    for (const brandSlug of brands) {
        ...
        if (!fs.existsSync(brandVaultDir)) {
            log(`Skipping ${brandSlug} — no vaults`);
            continue;
        }
        const brandExport = {};
        for (let v = 1; v <= 8; v++) {
            ...
            if (!fs.existsSync(vaultDir)) {
                brandExport[`vault-${v}-${vaultNames[v]}`] = [];
                continue;
            }
        ...
        const vault6 = brandExport["vault-6-creative-drafts"];
