...
export function diagnoseSwitchFromContext(context) {
    const lower = context.toLowerCase();
    if (lower.includes("no offer") || lower.includes("don't know what to sell") || lower.includes("unclear"))
        return 1;
    if (lower.includes("no audience") || lower.includes("no followers") || lower.includes("nobody knows"))
        return 2;
    if (lower.includes("no traffic") || lower.includes("no visitors") || lower.includes("no views"))
        return 3;
    if (lower.includes("no trust") || lower.includes("not converting") || lower.includes("people visit but"))
        return 4;
    if (lower.includes("warm leads") || lower.includes("not closing") || lower.includes("objections"))
        return 5;
    if (lower.includes("no repeat") || lower.includes("no referrals") || lower.includes("one-time buyers"))
        return 6;
    return 7;
}
