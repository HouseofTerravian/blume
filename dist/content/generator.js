...
export async function generatePost(req) {
    ...
    const mode = req.mode ?? "collaborative";
...
export async function generateAIDASequence(brandSlug, platform, goal, mode = "collaborative", topic) {
...
export async function generateOffer(brandSlug, offerNum) {
...
export async function generateEmail(brandSlug, sequenceType) {
    const brand = loadBrand(brandSlug);
...
export async function diagnoseBrand(brandSlug, context) {
    const brand = loadBrand(brandSlug);
    const switchNum = diagnoseSwitchFromContext(context);
    const sw = SEVEN_SWITCHES[switchNum];
    const advice = await think(`You are BLUME, Chief of Growth. You are advising on the brand "${brand.name}".
     You use the 7 Sales Switches™ framework to diagnose bottlenecks and prescribe action.
     Be specific, direct, and actionable. 3–5 sentences max.`, `Brand: ${brand.name}
     Context: ${context}
