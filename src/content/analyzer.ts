...
export async function analyzeSiteForBrand(url: string, brandSlug: string): Promise<StrategicInsight> {
...
Body Text Excerpt: ${site.bodyText.slice(0, 1500)}
  ...
  let parsed: { summary: string; opportunities: string[]; recommendedActions: string[]; suggestedPosts: string[] };
  try {
