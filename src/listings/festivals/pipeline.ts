...
export async function ingestFestivals(payload: {
  ...
  workflow_id?: string;
  triggered_by?: string;
}): Promise<{ ingested: number; skipped: number; batch_id: string }> {
  ...
  await dbEmitEvent(
    "festival_ingested",
    { batch_id, count: inserted.length, brand: brand ?? "festivalcalendar" },
    { brand: brand ?? "festivalcalendar", source: "blume", workflow_id }
  );
...
export async function dedupFestivalBatch(payload: {
  ...
  workflow_id?: string;
  triggered_by?: string;
}): Promise<{ checked: number; unique: number; duplicates: number; batch_id: string }> {
  const { batch_id, brand, workflow_id, triggered_by } = payload;

  if (!batch_id) throw new Error("dedupFestivalBatch requires batch_id.");
...
export async function generateFestivalPageSEO(payload: {
  ...
  if (!festival_id) throw new Error("generateFestivalPageSEO requires festival_id.");

  const festival = await dbGetFestivalById(festival_id);
  if (!festival) throw new Error(`Festival ${festival_id} not found.`);

  let seo = festival.seo_content as (Record<string, unknown> & { confidence?: string }) | null;
  if (!seo) {
...
export async function publishFestival(payload: {
  ...
  brand?: string;
  workflow_id?: string;
}): Promise<{ published: number; festival_id?: string; festival_name?: string; batch_id?: string }> {
  ...
  if (festival_id) {
    ...
    await dbEmitEvent(
      "festival_published",
      { festival_id, festival_name: festival.name, city: festival.city, category: festival.category, batch_id: batch_id ?? null },
      { brand: effectiveBrand, source: "blume", workflow_id }
    );
