...
- **`@terravian/blume` extraction:** terravian-mcp's persona/switches/seo/vault are now re-export shims (logic deduped to the package).
- **Wave-2 Lotus depth:** `lotus_bottleneck` + `lotus_missing_evidence` — score → guidance.
- **S5 Recommendation:** `recommend_next` — Lotus + 7 Switches → prioritized "what to do next" plan. **The doctrinal loop is complete: record (Spine) → score (Lotus) → guide (depth) → recommend (S5).**

## ▶ Candidate next tasks (pick one; gate with *"Does this accelerate value now?"*)
1. **Apply `migrations/0001_artifacts.sql`** to live Supabase (`wxinipsficonhfifjqek`) → enable the `thq_artifacts` mirror (turns the loop on for real data).
2. **Proof-of-Use on real publishes** — terravian-mcp posting path → artifact (now easier post-extraction).
3. **BLUME-032** — migrate legacy vault-entry tools (1–8) to the 12-slug registry.
4. **S8 Memory vault** — unblocks BLUME-082 (memory-compounded recommendations).
5. **Health Bar / investor summary** — remaining Lotus presentation surface.
